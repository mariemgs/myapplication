...
def create_fix_pr_with_gh(validated_report, security_review, solid_review, optimization):
    """Create fix PR autonomously using gh CLI — no human intervention needed"""
    import subprocess
    print("Creating fix PR autonomously using gh CLI...")

    fixes_content = "# AI Fix Suggestions for PR #{}

".format(PR_NUMBER)
    fixes_content += "## Validated Issues
{}

".format(validated_report[:2000])
    fixes_content += "## Security Review
{}

".format(security_review[:1000])
    fixes_content += "## SOLID Review
{}

".format(solid_review[:1000])
    fixes_content += "## Optimization
{}

".format(optimization[:1000])
    fixes_content += "---
...
"
    ...
    pr_body = "## AI-Generated Fix Suggestions

"
    pr_body += "> All issues validated — hallucinations removed.

"
    pr_body += "### Validated Issues
{}
